...
public:
    ...
    JSValue *invoke( const char *name, ExecState* exec, const List& args)
    {
        printf("invoke objet 2\n");
        if (!strcmp(name, "log") ) {
            JSType type = args.at(0)->type();
            if (args.size() == 1 && type == StringType)
            {
...
public:
    ...
    { 
        if (!strcmp(name, "logMessage") ) {
            ...
            if( type == NumberType )
            {
                intValue = (int)args.at(0)->toNumber(exec);
    ...
    JSValue *getProperty( const char * name, ExecState* exec) 
    {
        ...
        else if( !strcmp( name, "myTest" ) )
        {
            ...
            return Instance::createRuntimeObject(Instance::BalLanguage, myObject2);
        }
        return jsUndefined();
    };
    void setProperty( const char *name, ExecState* exec, const JSValue *value) 
    {
        ...
        {
            intValue = (int)value->toNumber(exec);
        }
    };
    void logMessage (const char *message)
    {
...
int main(int argc, char **argv)
{
    ...
    bool ret = true;
    {
        ...
        global->put(exec, Identifier("myInterface"), Instance::createRuntimeObject(Instance::BalLanguage, (void *)myObject));
