...
JSValue *Window::getValueProperty(ExecState *exec, int token) const
{
   ...
   case DOMException:
    ...
    case TvCore_:
        if( !d->m_tvcore )
		{
            d->m_tvcore = new TvCore(exec, impl()->frame());
		}
		else
		{
	...
	const_cast<Window *>(this)->putDirect("tvCore", d->m_tvcore, DontDelete|ReadOnly);
    ...
    case DvbCore_:
        if( !d->m_dvbcore )
            d->m_dvbcore = new DvbCore(exec, impl()->frame());
	// set attribute, or else dvbCore will be garbage collected
	const_cast<Window *>(this)->putDirect("dvbCore", d->m_dvbcore, DontDelete|ReadOnly);
