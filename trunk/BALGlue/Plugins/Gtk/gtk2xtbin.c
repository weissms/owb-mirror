...
GtkWidget*
gtk_xtbin_new (GtkWidget *container/*GdkWindow *parent_window*/, String * f)
{
  GtkXtBin *xtbin;
  gpointer user_data;

//   assert(parent_window != NULL);
  assert(container->window != NULL);
...
//   xtbin->parent_window = parent_window;
  xtbin->parent_window = container->window;
  ...
  gtk_widget_set_parent_window(GTK_WIDGET(xtbin), /*parent_*/container->window);
//   gdk_window_get_user_data(xtbin->parent_window, &user_data);
  if (user_data)
    gtk_container_add(GTK_CONTAINER(container/*user_data*/), GTK_WIDGET(xtbin));
