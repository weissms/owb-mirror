...
static void save_password_callback(SoupMessage* msg, WebKitAuthData* authData)
{
    ...
    g_signal_handlers_disconnect_by_func(msg, (void*)save_password_callback, authData);
...
static void detach(SoupSessionFeature* manager, SoupSession* session)
{
    g_signal_handlers_disconnect_by_func(G_OBJECT(session), (void*)session_authenticate, manager);
}
