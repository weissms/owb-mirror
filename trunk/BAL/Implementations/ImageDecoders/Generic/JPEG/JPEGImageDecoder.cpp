...
{
public:
    JPEGImageReader(BCJPEGImageDecoder* decoder)
        : m_decoder(decoder)
        , m_bufferLength(0)
        , m_bytesToSkip(0)
        , m_state(JPEG_HEADER)
    ...
    BCJPEGImageDecoder* decoder() { return m_decoder; }

private:
    BCJPEGImageDecoder* m_decoder;
...
BCJPEGImageDecoder::BCJPEGImageDecoder()
: m_reader(0)
{}
...
void BCJPEGImageDecoder::setData(const Vector<char>* data, bool allDataReceived)
{
...
bool BCJPEGImageDecoder::isSizeAvailable() const
{
...
RGBA32Buffer* BCJPEGImageDecoder::frameBufferAtIndex(size_t index)
{
    if (index)
        return 0;
...
void BCJPEGImageDecoder::decode(bool sizeOnly) const
{
    if (mImageDecoderCommonImplementation.failed())
        return;
...
bool BCJPEGImageDecoder::outputScanlines()
{
    if (mImageDecoderCommonImplementation.getFrameBufferCache().isEmpty())
        return false;
    ...
    unsigned* dst = buffer.bytes().data() + info->output_scanline * mImageDecoderCommonImplementation.size().width();
...
void BCJPEGImageDecoder::jpegComplete()
{
    if (mImageDecoderCommonImplementation.getFrameBufferCache().isEmpty())
        return;
