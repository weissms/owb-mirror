...
{
public:
    PNGImageReader(BCPNGImageDecoder* decoder)
...
void BCPNGImageDecoder::setData(const Vector<char>* data, bool allDataReceived)
{
...
bool BCPNGImageDecoder::isSizeAvailable() const
{
...
RGBA32Buffer* BCPNGImageDecoder::frameBufferAtIndex(size_t index)
{
    if (index)
        return 0;
    ...
    RGBA32Buffer& frame = mImageDecoderCommonImplementation.getFrameBufferCache()[0];
...
void BCPNGImageDecoder::decode(bool sizeOnly) const
{
    if (mImageDecoderCommonImplementation.failed())
        return;
    ...
    if (mImageDecoderCommonImplementation.failed() || (!pImpl->getFrameBufferCache().isEmpty() && pImpl->getFrameBufferCache()[0].status() == RGBA32Buffer::FrameComplete)) {
        delete m_reader;
        m_reader = 0;
    }
}
...
{
    static_cast<BCPNGImageDecoder*>(png_get_progressive_ptr(png))->decodingFailed();
...
{
    static_cast<BCPNGImageDecoder*>(png_get_progressive_ptr(png))->headerAvailable();
}

void BCPNGImageDecoder::headerAvailable()
{
    ...
    if (png->width > cMaxPNGSize || png->height > cMaxPNGSize) {
        mImageDecoderCommonImplementation.setFailed( true );
        longjmp(png->jmpbuf, 1);
        return;
    }
    ...
    if (reader()->decodingSizeOnly()) {
        ...
        reader()->setReadOffset(mImageDecoderCommonImplementation.data()->size() - png->buffer_size);
        png->buffer_size = 0;
    }
}
...
{
    static_cast<BCPNGImageDecoder*>(png_get_progressive_ptr(png))->rowAvailable(rowBuffer, rowIndex, interlacePass);
}

void BCPNGImageDecoder::rowAvailable(unsigned char* rowBuffer, unsigned rowIndex, int interlacePass)
{
    if (mImageDecoderCommonImplementation.getFrameBufferCache().isEmpty())
        return;
    ...
    if (buffer.status() == RGBA32Buffer::FrameEmpty) {
        ...
        buffer.setRect(IntRect(0, 0, mImageDecoderCommonImplementation.size().width(), mImageDecoderCommonImplementation.size().height()));

        if (reader()->pngPtr()->interlaced)
            reader()->createInterlaceBuffer((reader()->hasAlpha() ? 4 : 3) * mImageDecoderCommonImplementation.size().width() * mImageDecoderCommonImplementation.size().height());
    }
    ...
    */
    ...
    if (interlaceBuffer) {
        row = interlaceBuffer + (rowIndex * colorChannels * mImageDecoderCommonImplementation.size().width());
...
{
    static_cast<BCPNGImageDecoder*>(png_get_progressive_ptr(png))->pngComplete();
}

void BCPNGImageDecoder::pngComplete()
{
    if (mImageDecoderCommonImplementation.getFrameBufferCache().isEmpty())
        return;
