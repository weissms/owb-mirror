...
struct GIFImageReader {
    BC::BCGIFImageDecoder* clientptr;
    ...
    GIFImageReader(BC::BCGIFImageDecoder* client = 0) {
        clientptr = client;
        state = gif_type;
        bytes_to_consume = 6;
        bytes_in_hold = 0;
        ...
        loop_count = WebCore::cAnimationNone;
        count = 0;
    }
    ...
    bool read(const unsigned char* buf, unsigned int numbytes, 
              BC::BCGIFImageDecoder::GIFQuery query = BC::BCGIFImageDecoder::GIFFullQuery, unsigned haltAtFrame = -1);
