...
void mediaPlayerPrivateSourceChangedCallback(GObject *object, GParamSpec *pspec, gpointer data)
{
    ...
    GstElement* elementPtr = element.get();
#else
    GstElement* elementPtr = (GstElement*) fastMalloc(sizeof(GstElement));
    g_object_get(mp->m_playBin, "source", &elementPtr, NULL);
#endif
    
    gst_object_replace((GstObject**) &mp->m_source, (GstObject*) elementPtr);

    if (WEBKIT_IS_WEB_SRC(elementPtr)) {
        Frame* frame = mp->m_player->frameView() ? mp->m_player->frameView()->frame() : 0;

        if (frame)
            webKitWebSrcSetFrame(WEBKIT_WEB_SRC(elementPtr), frame);
    }
}
...
#else
        GError* error = (GError*) fastMalloc(sizeof(GError));
...
void MediaPlayerPrivate::load(PassRefPtr<Channel> channel)
{
    ...
    // FIXME: We will do a full scan each time a channel has no internal info. We should do it only sometimes.
    if (!info) {
        fprintf(stderr, "no channel internal info\n");
        return;
    }

    info = channel->channelInternalInfo();
    if (!info) {
        fprintf(stderr, "Could not play channel %s, could not find internal information. Check your configuration file!\n", channel->ccid().utf8().data());
        return;
    }
    // This avoids some issues with the player where the previous video just keeps playing
    // or prevent us from playing.
    cancelLoad();

    if (info->type() == ChannelInternalInfo::Local) {
        LocalChannelInternalInfo* localInfo = static_cast<LocalChannelInternalInfo*>(info);
        load(localInfo->path());
    }
    play();
}
