...
#if ENABLE(CEHTML)
#include "CEHTMLCSSDefinitions.h"
#endif

#if ENABLE(DAE)
#include "DAECSSDefinitions.h"
#endif
...
PassRefPtr<CSSValue> CSSComputedStyleDeclaration::getPropertyCSSValue(int propertyID, EUpdateLayout updateLayout) const
{
    ...
    switch (static_cast<CSSPropertyID>(propertyID)) {
        ...
        default:
#if ENABLE(CEHTML)
        if (isCEHTMLCSSProperty(propertyID))
            return getCEHTMLPropertyCSSValue(propertyID, updateLayout);
#endif
#if ENABLE(DAE)
        if (isDAECSSProperty(propertyID))
            return getDAEPropertyCSSValue(propertyID, updateLayout);
#endif
#if ENABLE(SVG)
