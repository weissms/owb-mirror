...
void runtimeObjectGetPropertyNames(ExecState* exec, PropertyNameArray& propertyNameArray, HTMLElement* element)
{
    RuntimeObject* runtimeObject = getRuntimeObject(exec, element);
    if (!runtimeObject)
        return;

    runtimeObject->getPropertyNames(exec, propertyNameArray);
}
