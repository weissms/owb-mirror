...
bool RenderObject::paintBorderImage(GraphicsContext* graphicsContext, int tx, int ty, int w, int h, const RenderStyle* style)
{
    ...
    if (drawLeft) {
        ...
        if (drawTop)
#ifdef __OWB__
            graphicsContext->drawImage(borderImage->image()->nativeImageForCurrentFrame(), IntRect(tx, ty, style->borderLeftWidth(), style->borderTopWidth()),
                                        IntRect(0, 0, leftSlice, topSlice));
            borderImage->image()->startAnimation();
#else
        ...
        if (drawBottom)
#ifdef __OWB__
            graphicsContext->drawImage(borderImage->image()->nativeImageForCurrentFrame(), IntRect(tx, ty + h - style->borderBottomWidth(), style->borderLeftWidth(), style->borderBottomWidth()),
...
#ifdef __OWB__
        graphicsContext->drawTiledImage(borderImage->image()->nativeImageForCurrentFrame(), IntRect(tx, ty + style->borderTopWidth(), style->borderLeftWidth(),
                                    h - style->borderTopWidth() - style->borderBottomWidth()),
                                    IntRect(0, topSlice, leftSlice, imageHeight - topSlice - bottomSlice),
                                    BIGraphicsContext::StretchTile, (BIGraphicsContext::TileRule)vRule);
        borderImage->image()->startAnimation();
#else
    ...
    if (drawRight) {
        ...
        if (drawTop)
#ifdef __OWB__
            graphicsContext->drawImage(borderImage->image()->nativeImageForCurrentFrame(), IntRect(tx + w - style->borderRightWidth(), ty, style->borderRightWidth(), style->borderTopWidth()),
        ...
        if (drawBottom)
#ifdef __OWB__
            graphicsContext->drawImage(borderImage->image()->nativeImageForCurrentFrame(), IntRect(tx + w - style->borderRightWidth(), ty + h - style->borderBottomWidth(), style->borderRightWidth(), style->borderBottomWidth()),
...
#ifdef __OWB__
        graphicsContext->drawTiledImage(borderImage->image()->nativeImageForCurrentFrame(), IntRect(tx + w - style->borderRightWidth(), ty + style->borderTopWidth(), style->borderRightWidth(),
                                    h - style->borderTopWidth() - style->borderBottomWidth()),
    ...
    if (drawTop)
#ifdef __OWB__
        graphicsContext->drawTiledImage(borderImage->image()->nativeImageForCurrentFrame(), IntRect(tx + style->borderLeftWidth(), ty, w - style->borderLeftWidth() - style->borderRightWidth(), style->borderTopWidth()),
                                    IntRect(leftSlice, 0, imageWidth - rightSlice - leftSlice, topSlice),
                                    (BIGraphicsContext::TileRule)hRule, BIGraphicsContext::StretchTile);
        borderImage->image()->startAnimation();
#else
    ...
    if (drawBottom)
#ifdef __OWB__
        graphicsContext->drawTiledImage(borderImage->image()->nativeImageForCurrentFrame(), IntRect(tx + style->borderLeftWidth(), ty + h - style->borderBottomWidth(),
    ...
    if (drawMiddle)
#ifdef __OWB__
        graphicsContext->drawTiledImage(borderImage->image()->nativeImageForCurrentFrame(), IntRect(tx + style->borderLeftWidth(), ty + style->borderTopWidth(), w - style->borderLeftWidth() - style->borderRightWidth(),
                                        h - style->borderTopWidth() - style->borderBottomWidth()),
                                        IntRect(leftSlice, topSlice, imageWidth - rightSlice - leftSlice, imageHeight - topSlice - bottomSlice),
                                        (BIGraphicsContext::TileRule)hRule, (BIGraphicsContext::TileRule)vRule);
        borderImage->image()->startAnimation();
#else
...
#ifdef __OWB__
BAL::BTAffineTransform RenderObject::localTransform() const
{
    return BAL::BTAffineTransform(1, 0, 0, 1, xPos(), yPos());
}

void RenderObject::setLocalTransform(const BAL::BTAffineTransform&)
{
    ASSERT(false);
}

BAL::BTAffineTransform RenderObject::absoluteTransform() const
{
    if (parent())
