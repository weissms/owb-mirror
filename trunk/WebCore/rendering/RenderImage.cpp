...
#ifdef __OWB__
				context->drawImage(image()->nativeImageForCurrentFrame(), IntPoint(tx + imageX, ty + imageY));
...
#ifdef __OWB__
		context->drawImage(image()->nativeImageForCurrentFrame(), rect, compositeOperator);
		image()->startAnimation();
#else
